...
def extract_text_from_pdf(file_path):
    """
    Максимально надежное извлечение текста из PDF с поддержкой кириллицы
    Цепочка методов: PyMuPDF (различные режимы) -> PyMuPDF OCR -> pdfplumber -> PyPDF2 -> OCR Tesseract
    """
...
def extract_text_from_dict(text_dict):
    """Извлекает текст из словаря PyMuPDF"""
    text_parts = []
    try:
...
def extract_text_from_blocks(blocks):
    """Извлекает текст из блоков PyMuPDF"""
    text_parts = []
    try:
        for block in blocks:
...
async def clear_analysis_history():
    """
    Очистка всей истории анализов пользователя
    """
    try:
