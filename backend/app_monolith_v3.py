...
class SimpleJWTManager:
    def __init__(self):
        self.secret_key = os.getenv("JWT_SECRET_KEY", "super-secret-jwt-key-for-devassist-pro")
...
# Simple user storage (in production should use database)
users_storage = {}
...
def create_user(user_data: UserCreate) -> Dict[str, Any]:
    ...
    if not password_manager.validate_password(user_data.password):
    ...
    users_storage[user_data.email] = user
    return user
...
async def get_current_user_simple(authorization: str = Depends(lambda: None)) -> Dict[str, Any]:
...
"""
    ...
    def extract_advanced_document_data(self, file_content: bytes, filename: str) -> Dict[str, Any]:
        """Extract advanced data from documents for V3 analysis"""
        try:
            extracted_data = {
                "tables": [],
                "currencies": [],
                    ...
                    with pdfplumber.open(pdf_buffer) as pdf:
                        ...
                        for pattern in currency_patterns:
                            ...
                            for match in matches:
                                extracted_data["currencies"].append(match)
            ...
            logger.info(f"📊 Advanced extraction completed: {len(extracted_data['tables'])} tables, {len(extracted_data['currencies'])} currency values")
            return extracted_data
            
        except Exception as e:
            logger.error(f"❌ Advanced extraction error: {e}")
            return {"tables": [], "currencies": [], "key_metrics": {}, "technical_specs": []}
    
    def _generate_enhanced_business_analysis(self, content: str) -> Dict[str, Dict[str, Any]]:
    ...
    def _calculate_weighted_score(self, business_analysis: Dict[str, Dict[str, Any]]) -> int:
    ...
    def _generate_recommendations(self, business_analysis: Dict) -> List[str]:
...
async def get_weight_presets():
    ...
    presets = {
        ...
        "budget_focused": {
            ...
            "description": "Фокус на экономической эффективности",
            "weights": CriteriaWeight(
        ...
        "technical_focused": {
            ...
            "description": "Акцент на техническом соответствии",
            "weights": CriteriaWeight(
                technical_compliance=0.30,
                functional_coverage=0.20,
                development_methodology=0.12
            )
        }
    }
    return presets
...
async def create_custom_weights(config: WeightConfigRequest):
    """Create custom criteria weights configuration"""
    if config.custom_weights:
        # Validate weights sum to 1.0
        total_weight = sum([
            config.custom_weights.budget_compliance,
            config.custom_weights.timeline_compliance,
            config.custom_weights.technical_compliance,
            config.custom_weights.team_expertise,
            config.custom_weights.functional_coverage,
            config.custom_weights.quality_assurance,
            config.custom_weights.development_methodology,
            config.custom_weights.scalability,
            config.custom_weights.communication,
            config.custom_weights.added_value
        ])
        
        if abs(total_weight - 1.0) > 0.01:
            raise HTTPException(status_code=400, detail="Weight sum must equal 1.0")
        
        return {"status": "success", "weights": config.custom_weights, "total_weight": total_weight}
    else:
        return {"status": "error", "message": "Custom weights required"}

@app.post("/api/v3/documents/upload")
async def upload_v3_document(
    file: UploadFile = File(...),
    current_user: Dict[str, Any] = Depends(get_current_user_simple)
):
    """Upload document for V3 analysis with advanced extraction"""
    try:
        # Validate file type
        ...
        extraction_data = ai_provider.extract_advanced_document_data(file_content, file.filename)
        
        # Store file content and metadata
        doc_id = len(file_content_storage) + 1
        ...
        return {
            ...
            "extraction_preview": {
                ...
                "has_technical_specs": len(extraction_data["technical_specs"]) > 0
            },
            "advanced_features": ["table_extraction", "currency_detection", "technical_analysis"]
        }
...
async def analyze_v3(request: V3AnalysisRequest, current_user: Dict[str, Any] = Depends(get_current_user_simple)):
    ...
    try:
        ...
        primary_doc = file_content_storage[request.document_ids[0]]
        content = primary_doc["content"].decode('utf-8', errors='ignore')
        
        # Get configuration
        weights = CriteriaWeight()
        if request.analysis_config:
            if request.analysis_config.custom_weights:
                weights = request.analysis_config.custom_weights
            elif request.analysis_config.preset == "budget_focused":
                weights = CriteriaWeight(budget_compliance=0.25, technical_compliance=0.15)
            elif request.analysis_config.preset == "technical_focused":
                weights = CriteriaWeight(technical_compliance=0.30, functional_coverage=0.20)
        
        # Perform advanced AI analysis
        ai_prompt = """
Проведите экспертный анализ коммерческого предложения по 10 критериям.
Верните результат в JSON формате со следующими полями:
- company_name: название компании
- overall_score: общая оценка (0-100)
- executive_summary: краткое резюме
- detailed_analysis: детальный анализ по каждому критерию
- recommendations: список рекомендаций
- risk_assessment: оценка рисков
"""
        
        ai_result = await ai_provider.analyze_with_claude(ai_prompt, content)
        
        # Extract advanced data
        extraction_data = primary_doc.get("extraction_data", {})
        
        # Generate charts if requested
        charts_data = {}
        if request.generate_charts:
            charts_data = generate_analysis_charts(ai_result, extraction_data)
        ...
        analysis_result = V3AnalysisResponse(
            ...
            status="completed",
            overall_score=ai_result.get("overall_score", 85),
            weighted_score=ai_result.get("weighted_score", 85.5),
            company_name=ai_result.get("company_name", "Анализируемая компания"),
            summary=ai_result.get("summary", "V3 экспертный анализ завершен"),
            executive_summary=ai_result.get("executive_summary", "Детальный анализ КП с применением 10-критериальной системы оценки"),
            recommendations=ai_result.get("recommendations", ["Анализ завершен успешно"]),
            criteria_weights=weights,
            risk_level=ai_result.get("risk_level", "Умеренный риск"),
            analysis_type="v3_expert",
            processing_time=processing_time,
            currency_data=extraction_data.get("currencies", []),
            extracted_tables=extraction_data.get("tables", []),
...
def generate_analysis_charts(analysis_data: Dict[str, Any], content_data: Dict[str, Any]) -> Dict[str, Any]:
    """Generate charts for V3 analysis visualization"""
    try:
        charts = {}
        
        # Criteria scores radar chart
        criteria_labels = ["Technical", "Budget", "Timeline", "Quality", "Innovation", "Risk"]
        criteria_values = [85, 78, 92, 80, 75, 88]  # Example values
...
async def get_v3_analysis_history(
    current_user: Dict[str, Any] = Depends(get_current_user_simple),
    skip: int = Query(0),
    limit: int = Query(10)
):
    """Get V3 analysis history for current user"""
    try:
        # Filter analyses for current user
        user_analyses = []
        for analysis_id, analysis in v3_analysis_storage.items():
...
async def export_v3_pdf(analysis_id: int, current_user: Dict[str, Any] = Depends(get_current_user_simple)):
    """Export V3 analysis to PDF with Cyrillic support"""
    try:
        if analysis_id not in v3_analysis_storage:
            raise HTTPException(status_code=404, detail="V3 analysis not found")
        ...
        pdf_data = {
            ...
            "analysis_type": "V3 Expert Analysis",
        ...
        # Try to generate PDF with tender style exporter
        try:
            from services.reports.core.tender_style_pdf_exporter import TenderStylePDFExporter
            
            pdf_exporter = TenderStylePDFExporter()
            pdf_buffer = await pdf_exporter.generate_kp_analysis_pdf(pdf_data)
            ...
            return {
                ...
                "fallback_data": "V3 analysis data available in JSON format",
                "alternative_formats": ["JSON", "Excel", "Web View"],
                "support_info": "Contact support for PDF generation assistance"
            }
...
# Authentication endpoints for V3
...
if __name__ == "__main__":
    ...
    print("Starting DevAssist Pro - Monolith Application with V3")
    print("=" * 50)
    ...
    print("   - V2 KP Analyzer:   http://localhost:8000/api/v2/kp-analyzer/")
    print("   - Admin Panel:      http://localhost:8000/api/admin/")
    print("   - PDF Export:       http://localhost:8000/api/reports/export/ (Cyrillic Support)")
    print("")
    print("🎯 V3 EXPERT ANALYSIS SYSTEM:")
    print("   - V3 Test:          http://localhost:8000/api/v3/test")
    ...
    print("   - Profile:          http://localhost:8000/api/auth/me")
