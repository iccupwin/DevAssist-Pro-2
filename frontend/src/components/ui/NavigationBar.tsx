    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50">
            <nav
                ...
                className="group px-2 transition-all duration-500 ease-in-out">
                <div className={cn(
                    'mx-auto mt-2 px-6 transition-all duration-500 ease-in-out lg:px-12',
                    // Обычное состояние
                    'max-w-6xl',
                    // При скролле
                    isScrolled && [
                        'max-w-4xl rounded-2xl border border-border/50',
                    ...
                    <div className="relative flex flex-wrap items-center justify-between gap-6 py-3 lg:gap-0 lg:py-4">
                        ...
                        <div className="flex w-full justify-between lg:w-auto">
                            ...
                            <button
                                ...
                                className="relative z-20 -m-2.5 -mr-4 block cursor-pointer p-2.5 lg:hidden transition-all duration-300 hover:bg-accent/50 rounded-md">
                                <Menu className="group-data-[state=active]:rotate-180 group-data-[state=active]:scale-0 group-data-[state=active]:opacity-0 m-auto size-6 duration-300 ease-in-out" />
                                <X className="group-data-[state=active]:rotate-0 group-data-[state=active]:scale-100 group-data-[state=active]:opacity-100 absolute inset-0 m-auto size-6 -rotate-180 scale-0 opacity-0 duration-300 ease-in-out" />
                            </button>
                        </div>
                            ...
                            <ul className="flex gap-8 text-sm">
                                {menuItems.map((item, index) => (
                                    <li key={index}>
                                        <a
                                            href={item.href}
                                            className="text-muted-foreground hover:text-foreground block duration-300 transition-all relative group">
                        ...
                        {/* Кнопки действий */}
                        <div className="bg-background/95 backdrop-blur-md group-data-[state=active]:block lg:group-data-[state=active]:flex mb-6 hidden w-full flex-wrap items-center justify-end space-y-8 rounded-3xl border border-border/50 p-6 shadow-2xl shadow-black/10 md:flex-nowrap lg:m-0 lg:flex lg:w-fit lg:gap-6 lg:space-y-0 lg:border-transparent lg:bg-transparent lg:p-0 lg:shadow-none dark:shadow-black/20 dark:lg:bg-transparent">
                            {/* Мобильное меню */}
                                ...
                                <ul className="space-y-6 text-base">
                                    {menuItems.map((item, index) => (
                                        <li key={index}>
                                            <a
                                                href={item.href}
                                                className="text-muted-foreground hover:text-foreground block duration-300 transition-all">
                                                <span>{item.name}</span>
                                            </a>
                                        </li>
                                    ))}
                            ...
                            <div className="flex w-full flex-col space-y-3 sm:flex-row sm:gap-3 sm:space-y-0 md:w-fit">
                                <Button
                                    ...
                                    size="sm"
                                    className={cn(
                                        'transition-all duration-300 hover:bg-accent/50 border-border/50',
