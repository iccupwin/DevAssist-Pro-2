/**
 * Working PDF Exporter - Ported from DevAssist-Pro-2-final
 * Proven Cyrillic support with @react-pdf/renderer
 * Based on working implementation that correctly displays Russian text
 */
...
// Working PDF Exporter Component
interface WorkingPDFExporterProps {
  analysisData: any;
  filename?: string;
  className?: string;
...
const WorkingPDFExporter: React.FC<WorkingPDFExporterProps> = ({
  analysisData,
  filename,
  className = '',
  children
  ...
  const handleExport = async () => {
    if (!analysisData) {
      console.error('No analysis data provided for PDF export');
      return;
    }
    ...
    try {
      ...
      const results: AnalysisResult[] = [{
        companyName: analysisData.companyName || 'Неизвестная компания',
        fileName: analysisData.documentName || 'Документ',
        analyzedAt: analysisData.createdAt || new Date().toISOString(),
        model: analysisData.aiModel || 'claude-3-haiku-20240307',
        overallRating: analysisData.overallScore || 0,
        strengths: analysisData.executiveSummary?.keyStrengths || [],
        weaknesses: analysisData.executiveSummary?.criticalWeaknesses || []
      }];

      const options: PDFExportOptions = {
        customTitle: 'Отчет по анализу коммерческого предложения',
        projectName: 'DevAssist Pro v2 - КП Анализатор',
      ...
      const blob = await generatePDF(results, undefined, options);
      const fileName = filename || `DevAssist_Pro_KP_Analysis_${Date.now()}.pdf`;
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${className}`}
    >
      {isGenerating ? (
        <>
...
export default WorkingPDFExporter;